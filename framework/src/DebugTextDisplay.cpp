...
void DebugTextDisplay::Initialize(ID3D11Device* device,
...
void DebugTextDisplay::Draw(const std::vector<std::wstring>& headline,
                            const std::vector<std::wstring>& detail)
{
    ...
    // Xbox overscan: keep everything well inside the panel so a TV does not clip it.
    ...
    // Presented without waiting for vsync. This is a diagnostic overlay, and blocking here
    // would pace whatever else the application is doing to the display's refresh rate -
    // which for a streaming host means silently capping the stream at the TV's frame rate.
